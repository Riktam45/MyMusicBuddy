...
async def stop_realtime_session(session_id: str = Form(...)):
    ...
    try:
        ...
        last_notes = state.get("last_notes", []) if state else []

        first_note = last_notes[0] if last_notes else None

        result = {
            "success": True,
            "event_complete": bool(
                state and state.get("active")
            ),
            "event_id": int(
            ...
            "note_with_octave": (
                first_note.get(
                    "note_with_octave",
                    f"{first_note.get('note', '')}"
                    f"{first_note.get('octave', '')}"
                )
                if first_note
                else None
            ),
        ...
        return {
            ...
            "suggestions": None,
            "message": "Could not build realtime suggestions.",
        }
